...
/* ./generate -i avx512_vpclmulqdq -p crc32c -a v3s2x4e */
/* MIT licensed */
...
uint32_t avx512_vpclmulqdq_v3s2x4e(uint32_t crc0, const uint8_t *data, size_t len)
{
  ...
  if (len >= 256)
  {
    ...
    crc0 = _mm_crc32_u64(crc0, *(const uint64_t *)(buf + 24));
    crc1 = _mm_crc32_u64(crc1, *(const uint64_t *)(buf + klen + 24));
    vc0 = crc_shift(crc0, klen + blk * 192);
    vc1 = crc_shift(crc1, 0 + blk * 192);
    ...
    crc0 = _mm_crc32_u64(crc0, vc ^ _mm_extract_epi64(z0, 1));
    buf = buf2;
    len = end - buf;
  }
